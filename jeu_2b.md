...
Choisis le même arrière-plan que celui dans l'indice.
...
La valeur ``||scroller: vx -25||`` permet à l'arrière-plan de défiler vers la gauche.

Remplace la valeur ``||scroller:-50||`` de droite par ``||scroller:0||``.

La valeur ``||scroller:vy 0||`` empêche le défilement vertical de l'arrière-plan.
...
Fais **retourner horizontalement** le lutin dans le même sens que l'indice à l'aide de l'onglet *** Éditeur *** dans la Galerie.
...
** Ajoute seulement le bloc manquant à ta séquence de programmation. **
...
Modifie le serpent d'une couleur de ton choix.

Attention tu dois colorier *toutes les images des animations*.

Utilise le sceau de peinture.

## Étape 10

Ajoute un écran titre à ton jeu.

** À quel endroit dans la pile ``||loops:au démarrage||`` dois-tu placer ce bloc ? **
